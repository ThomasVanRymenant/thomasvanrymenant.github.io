...
export function getQuantityDatePairs(artikelObj) {
    // console.log(artikelObj);
    const pairs = [], quantityKeyRegexp = /^Tal[\d]{1,}$/, pairNrIdentifierRegexp = /\d{1,}$/; // const expDateKeyRegexp = /^Datum[\d]{1,}$/;
    for (let quantityKey of Object.keys(artikelObj).filter(k => quantityKeyRegexp.test(k))) { // iterate over keys which are holding a quantity
        if (Number(artikelObj[quantityKey]) != NaN && Number(artikelObj[quantityKey]) > 0) {
            // store quantity/expiration-date pair
            pairs.push([artikelObj[quantityKey], formatDate(artikelObj[`Datum${quantityKey.match(pairNrIdentifierRegexp)}`])]);
        }
    }
    return pairs
}
...
export function clearInvalidQuantityCells(articleObj) {
    ...
    const quantityKeyNames = Object.keys(articleObj).filter(key=>quantityKeyRegexp.test(key));
...
export const convertExcelToArrayOfObjects = (file) => new Promise((resolve, reject) => {
    if (!XLSXisAvailable()) {
        // alert('Er ging iets mis. Check de wifi connectie en probeer opnieuw');
        // throw new Error('Er ging iets mis. Check de wifi connectie en probeer opnieuw');
        reject(new Error('Er ging iets mis. Check of er wifi verbinding is en probeer opnieuw'));
    }
    const reader = new FileReader();
    reader.onload = (event) => {
        // let workbook = XLSX.read(event.target.result, {type:"binary",cellText:false,cellDates: true});
...
// promise-equivalent of the function
    ...
    try {
        ...
        for (let i = 0, quantDatePairs = [], newArticle = undefined; i < L; i++) {
            ...
            if ('__EMPTY' in excelRows[i] && /[0-9]{7}\s-/.test(excelRows[i]['__EMPTY'])) {
                ...
                // if already found an article-row (in format: 7digits - ....), row must be a 'quantity-date' row
                if (articles.length > 0) {
                    // add quantity/date pair to pairs
...
// custom function which formats excel-file-row-object (from excel file that is extracted from exactonline) into a workable format
export function extractArticleObjectsFromExactonlineExcelFile(excelRows) {
    const L = excelRows.length;
    let articles = [];
